...
@Test func testTrackDisplayName() async throws {
    // Test that displayName returns title when available
    let trackWithTitle = Track(
        ...
        comment: nil,
        grouping: nil,
        location: "/iPod_Control/Music/test.mp3",
        duration: 180,
        fileSize: 1024,
        bitrate: 128,
        sampleRate: 44100,
    ...
    #expect(trackWithTitle.displayName == "Test Song")
}

@Test func testTrackDisplayNameFromLocation() async throws {
    // Test that displayName falls back to filename when no title
    let trackWithoutTitle = Track(
        ...
        comment: nil,
        grouping: nil,
        location: "/iPod_Control/Music/MySong.mp3",
        duration: 180,
        fileSize: 1024,
        bitrate: 128,
        sampleRate: 44100,
    ...
    #expect(trackWithoutTitle.displayName == "MySong")
}

@Test func testTrackDurationFormatting() async throws {
    ...
    #expect(shortTrack.durationFormatted == "3:23")
    ...
    #expect(longTrack.durationFormatted == "1:02:03")
}

@Test func testTrackPlaybackFlags() async throws {
    ...
    #expect(playedTrack.hasBeenPlayed == true)
    #expect(playedTrack.hasBeenSkipped == true)
    #expect(playedTrack.hasRating == true)
    #expect(playedTrack.hasBookmark == true)

    let unplayedTrack = Track(
        id: 2,
        index: 1,
        title: "Unplayed Song",
        artist: nil,
    ...
    #expect(unplayedTrack.hasBeenPlayed == false)
    #expect(unplayedTrack.hasBeenSkipped == false)
    #expect(unplayedTrack.hasRating == false)
    #expect(unplayedTrack.hasBookmark == false)
}

@Test func testTrackPlayToSkipRatio() async throws {
    // Track with plays but no skips
    let allPlays = Track(
        id: 1,
        index: 0,
        title: "Popular",
    ...
    #expect(allPlays.playToSkipRatio == .infinity)

    // Track with plays and skips
    let mixedTrack = Track(
        id: 2,
        index: 1,
        title: "Mixed",
        artist: nil,
    ...
    #expect(mixedTrack.playToSkipRatio == 2.0)

    // Track with no plays
    let neverPlayed = Track(
        id: 3,
        index: 2,
        title: "Never Played",
        artist: nil,
    ...
    #expect(neverPlayed.playToSkipRatio == 0)
}
...
@Test func testiPodFacadeWithITunesDB() async throws {
    ...
    #expect(!firstTrack.displayName.isEmpty, "Track should have a display name")
